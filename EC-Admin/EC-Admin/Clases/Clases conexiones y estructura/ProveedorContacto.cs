...
namespace EC_Admin
{
    public class ProveedorContacto : PersonaContacto
    {
        #region Eventos
        ...
        private static void Cant()
        {
            try
            {
                string sql = "SELECT COUNT(id) AS c FROM contacto_proveedor";
        ...
        /// <summary>
        /// Inicializa la instancia de clase de ProveedorContacto y obtiene los contactos asociados al proveedor
        /// </summary>
        /// <param name="idPersona">ID del proveedor</param>
        public ProveedorContacto(int idPersona)
        {
            this.idP = idPersona;
            ObtenerContactos();
        }
        ...
        /// <summary>
        /// Método que limpia las listas con los datos de todos los contactos de determinado proveedor
        /// </summary>
        ...
        public override void ObtenerDatosContacto()
        {
            try
            {
                ...
                sql.CommandText = "SELECT * FROM contacto_proveedor WHERE id=?id";
        ...
        /// <summary>
        /// Obtiene todos los contactos asociados con determinado proveedor
        /// </summary>
        protected override void ObtenerContactos()
        {
            ...
            try
            {
                LimpiarListas();
                MySqlCommand sql = new MySqlCommand();
                sql.CommandText = "SELECT * FROM contacto_proveedor WHERE id_proveedor=?id_proveedor";
                sql.Parameters.AddWithValue("?id_proveedor", idP);
                DataTable dt = ConexionBD.EjecutarConsultaSelect(sql);
                foreach (DataRow dr in dt.Rows)
                {
                    idCS.Add((int)dr["id"]);
        ...
        public override void Insertar()
        {
            ...
            {
                MySqlCommand sql = new MySqlCommand();
                sql.CommandText = "INSERT INTO contacto_proveedor (id_proveedor, nombre, apellidos, telefono1, telefono2, extension, celular, lada1, lada2, email, create_user, create_time) " +
                    "VALUES (?id_proveedor, ?nombre, ?apellidos, ?telefono1, ?telefono2, ?extension, ?celular, ?lada1, ?lada2, ?email, ?create_user, NOW())";
                sql.Parameters.AddWithValue("?id_proveedor", idP);
        ...
        public override void Editar()
        {
            ...
            {
                MySqlCommand sql = new MySqlCommand();
                sql.CommandText = "UPDATE contacto_proveedor SET nombre=?nombre, apellidos=?apellidos, telefono1=?telefono1, telefono2=?telefono2, " +
        ...
        public void Eliminar(int id)
        {
            ...
            {
                MySqlCommand sql = new MySqlCommand();
                sql.CommandText = "DELETE FROM contacto_proveedor WHERE id=?id";
                sql.Parameters.AddWithValue("?id", id);
                ConexionBD.EjecutarConsulta(sql);
                OnContactDataBaseModified(new EventArgs());
            }
