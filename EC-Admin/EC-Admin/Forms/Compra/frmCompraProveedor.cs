...
namespace EC_Admin.Forms
{
    public partial class frmCompraProveedor : Form
    {
        ...
        private void Buscar(string p)
        {
            try
            {
                string sql = "SELECT id, nombre, razon_social, telefono1, telefono2, email, lada1, lada2 FROM proveedor WHERE nombre LIKE '%" + p + "%' OR razon_social LIKE '%" + p + "%' AND sucursal_id='" + Config.idSucursal + "'";
