...
namespace EC_Admin.Forms
{
    public partial class frmContactosCliente : Form
    {
        ...
        private void LlenarPanel()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < c.IDCS.Count; i++)
                {
                    ...
                    if (c.CorreoContactos[i] != "")
                        correo = c.CorreoContactos[i];
                    //Asignamos sus propiedades usando el método PropiedadesLabel
                    PropiedadesLabel(ref lblNombre, "lblNombre" + i.ToString("000"), c.NombreContactos[i], new Point(lNom, y), tabIndex);
                    tabIndex++;
        ...
        private string Telefonos(int pos)
        {
            ...
            try
            {
                if (c.TelefonoContactos01[pos] != "" && c.TelefonoContactos02[pos] != "")
                {
                    if (c.LadaContactos01[pos] != "")
                    {
                        tel += c.LadaContactos01[pos] + " " + c.TelefonoContactos01[pos];
                    }
                    else
                    {
                        tel += c.TelefonoContactos01[pos];
                    }
                    if (c.LadaContactos02[pos] != "")
                    {
                        tel += ", " + c.LadaContactos02[pos] + " " + c.TelefonoContactos02[pos];
                    }
                    else
                    {
                        tel += ", " + c.TelefonoContactos02[pos];
                    }
                }
                else if (c.TelefonoContactos01[pos] != "")
                {
                    if (c.LadaContactos01[pos] != "")
                    {
                        tel += c.LadaContactos01[pos] + " " + c.TelefonoContactos01[pos];
                ...
                else if (c.TelefonoContactos02[pos] != "")
                {
                    if (c.LadaContactos02[pos] != "")
                    {
                        tel += c.LadaContactos02[pos] + " " + c.TelefonoContactos02[pos];
